...
def LoadSAPData():
    ...
    now = datetime.now()
    # CheckStatus = Worker.Check_Status("SAP")
    ...
    RUNING = True
    # status = Worker.Load_Data()
        
    # if not status:
    #     Worker.Insert_SQL(f"insert into ACWO.dbo.DataStatus (SYSTEM, TIME) values ('SAP','error')")
    #     RUNING = False
    #     return jsonify({
    #         "error": "Export erorr",
    #     }), 429
    ...
    RUNING = False
    # Worker.Insert_SQL(f"insert into ACWO.dbo.DataStatus (SYSTEM, TIME) values ('SAP','{now.strftime("%H:%M %d-%m-%Y")}')")
    return jsonify({
        "status": "done",
...
@app.route("/api/int211", methods=["POST"])
def Run():
    log.info("Run INT 211")
    global LAST_REQUEST_TIME3, RUNING
    now = datetime.now()
    CheckStatus = Worker.Check_Status("INT189")
